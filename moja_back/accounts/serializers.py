...
class CustomRegisterSerializer(RegisterSerializer):
    ...
    def get_cleaned_data(self):
        print(self.validated_data.get('password1', ''))
        print(self.validated_data.get('password2', ''))
        print(self.validated_data.get('bank', ''))
        print(self.validated_data.get('bank', ''))
        return {
        'username': self.validated_data.get('username', ''),
        'password1': self.validated_data.get('password1', ''),
        'password2': self.validated_data.get('password2', ''),  # 추가 확인
...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        exclude = ['password', 'last_login', 'first_name', 'last_name', 'is_staff', 'is_active', 'groups', 'user_permissions']
